daily_query='''
DECLARE @today_3am datetime = DATEADD(hour, 3, CONVERT(datetime, CONVERT(date, GETDATE())))
...
SELECT
	...
	--,lwmain.geox																										AS pointx
	--,lwmain.geoy																										AS pointy
...
WHERE 
	lwmain.inci_id IS NOT NULL
	AND lwmain.geox != 0
	AND lwmain.geoy != 0
	AND lwmain.date_rept >= DATEADD(day, -2, GETDATE())  -- get records from yesterday
    AND lwmain.date_rept < @today_3am  -- get records up to 3am today

	AND lwmain.lwmainid NOT IN (select lwmainid from dbo.df_sent)
'''